    ...
    try {
      const outputImages1 = pdf2img.convert(urlData).catch((err) => {
        ...
        return res
          ...
          .json({
            Error:
              "Error upoading file. Please ensure the file name only contains anlphabets, numbers, spaces and dashes",
          });
      });
          ...
          try {
            ...
            if (insertBookReturn === true) {
              for (var i = 0; i < outputImages.length; i++) {
                ...
                var {
                  data: { text },
                } = await worker.recognize("./output" + fileName + i + ".png");

                console.log(
                  "This is the text from page " + (i + 1) + " : " + text
                );
                ...
                console.log(
                  "This is the whole embedding data: " + pdfEmbedding
                );
            ...
            console.log(
              "Error extracting and storing data from each text : " + error
            );
            return res
              .status(404)
              .json({ message: "Check your network connection and try again" });
          }
        }
      });
    } catch (err) {
